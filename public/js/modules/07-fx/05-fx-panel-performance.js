var homeWaveTrackState = { bars: 0, smooth: [], energy: 0, bass: 0, beat: 0, lastAt: 0 };
...
function updateHomeAudioVisual(dt) {
  ...
  var heroEl = document.querySelector('#empty-home .home-hero') || document.querySelector('.home-hero');
  var isLive = !!(typeof playing !== 'undefined' && playing && typeof audio !== 'undefined' && audio && !audio.paused);
  var se = (typeof smoothEnergy !== 'undefined' ? smoothEnergy : 0);
  var sb = (typeof smoothBass !== 'undefined' ? smoothBass : 0);
  var bp = (typeof beatPulse !== 'undefined' ? beatPulse : 0);
  var targetEnergy = isLive && typeof clampRange === 'function' ? clampRange(se * 0.76 + bp * 0.18, 0, 1) : 0;
  var targetBass = isLive && typeof clampRange === 'function' ? clampRange(sb * 0.92 + bp * 0.16, 0, 1) : 0;
  var targetBeat = isLive && typeof clampRange === 'function' ? clampRange(bp * 1.15, 0, 1) : 0;
  homeWaveTrackState.energy += (targetEnergy - homeWaveTrackState.energy) * (targetEnergy > homeWaveTrackState.energy ? 0.32 : 0.10);
  homeWaveTrackState.bass += (targetBass - homeWaveTrackState.bass) * (targetBass > homeWaveTrackState.bass ? 0.34 : 0.11);
  homeWaveTrackState.beat += (targetBeat - homeWaveTrackState.beat) * (targetBeat > homeWaveTrackState.beat ? 0.42 : 0.16);
  if (heroEl) {
    heroEl.style.setProperty('--home-audio-energy', homeWaveTrackState.energy.toFixed(3));
    heroEl.style.setProperty('--home-audio-bass', homeWaveTrackState.bass.toFixed(3));
    heroEl.style.setProperty('--home-audio-beat', homeWaveTrackState.beat.toFixed(3));
  }

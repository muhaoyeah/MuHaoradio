...
function normalizePlaylistProvider(provider) {
  if (provider === 'qq' || provider === 'kugou' || provider === 'kugou-lite' || provider === 'qishui' || provider === 'spotify') return provider;
  return 'netease';
}
function playlistProviderLabel(provider) {
  provider = normalizePlaylistProvider(provider);
  return provider === 'qq' ? 'QQ' : (provider === 'kugou-lite' ? '概念' : (provider === 'kugou' ? 'KG' : (provider === 'qishui' ? 'QS' : (provider === 'spotify' ? 'SP' : 'NE'))));
}
function playlistProviderName(provider) {
  provider = normalizePlaylistProvider(provider);
  if (provider === 'spotify') return 'Spotify';
  return provider === 'qq' ? 'QQ 音乐' : (provider === 'kugou-lite' ? '酷狗概念版' : (provider === 'kugou' ? '酷狗音乐' : (provider === 'qishui' ? '汽水音乐' : '网易云音乐')));
}
...
function playlistPanelProviderId(provider, id) {
  ...
  if (provider === 'kugou-lite') return 'kugou-lite:' + id;
...
function playlistTracksEndpoint(provider, id, params) {
  ...
  if (provider === 'kugou-lite') return '/api/kugou-lite/playlist/tracks?' + query;
...
function playlistPanelBuildVirtualEntries() {
  ...
  var labels = { netease: '网易云歌单', qq: 'QQ 音乐歌单', 'kugou-lite': '酷狗概念版歌单', kugou: '酷狗音乐歌单', qishui: '汽水音乐歌单', spotify: 'Spotify 歌单' };
  var order = ['netease', 'qq', 'kugou-lite', 'kugou', 'qishui', 'spotify'];
  var groups = { netease: [], qq: [], 'kugou-lite': [], kugou: [], qishui: [], spotify: [] };

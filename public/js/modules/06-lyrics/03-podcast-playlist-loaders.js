...
function renderMyPodcastRadioItems(key, title, items) {
  ...
  $pod.innerHTML = '<div class="podcast-inline-head"><div class="pl-section-label">' + escHtml(title || '我的播客') + '</div><button class="fx-mini-btn ghost" data-podcast-back="1" style="height:24px;padding:0 9px;font-size:10.5px">返回</button></div>' +
    items.map(function (r) {
      var thumb = r.cover ? safeImgSrc(coverUrlWithSize(r.cover, 88)) : '';
      ...
      return '<div class="pl-card podcast-card podcast-child" data-podcast-radio-id="' + escapeAttr(String(r.id || r.radioId || '')) + '" data-podcast-title="' + escapeAttr(r.name || '') + '">' +
        imgTag +
...
function playlistQueueSource(id) {
  ...
  if (raw.indexOf('kugou-lite:') === 0) return { provider: 'kugou-lite', id: raw.slice(11), requestId: raw };
...
function playlistQueuePageSize(provider, initial) {
  if (initial) return provider === 'kugou' || provider === 'kugou-lite' || provider === 'qishui' ? 50 : (provider === 'spotify' ? 96 : PLAYLIST_QUEUE_INITIAL_BATCH_SIZE);
  if (provider === 'kugou' || provider === 'kugou-lite' || provider === 'qishui') return 50;
